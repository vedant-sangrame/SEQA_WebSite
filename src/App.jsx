...
export default function App() {
  ...
  const [currentUser, setCurrentUser] = useState(() => {
    ...
    return {
      email: savedEmail || '',
      role: 'Admin', // Default Admin role for main link visitor
    };
  });
  ...
  // Save current user email & sync to backend
  const handleUserChange = async (newEmail, forcedRole = null) => {
    if (!newEmail || !newEmail.trim()) return;
    const cleanEmail = newEmail.trim();
    const roleToUse = forcedRole || currentUser.role || 'Admin';
    ...
    try {
      await fetch('/api/members/join', {
        ...
        body: JSON.stringify({ email: cleanEmail, role: roleToUse }),
      });
      fetchMembers();
    } catch (err) {
  ...
  // URL invite token resolution & Admin default logic
  useEffect(() => {
    ...
    if (inviteToken && invitedEmail) {
      fetch(`/api/invitations/verify?token=${encodeURIComponent(inviteToken)}&email=${encodeURIComponent(invitedEmail)}`)
        .then((res) => res.json())
        .then((data) => {
          const role = data.role || 'Member';
          handleUserChange(invitedEmail, role);
          setInviteNotification(`Joined via email invitation as ${invitedEmail} (${role})`);
        })
        .catch(() => {
          handleUserChange(invitedEmail, 'Member');
        });
    } else {
      const storedEmail = localStorage.getItem('sprintsync_user_email');
      if (storedEmail) {
        handleUserChange(storedEmail, 'Admin');
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on('member_role_updated', ({ email, role }) => {
      setMembers((prev) =>
