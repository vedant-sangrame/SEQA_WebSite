...
export default function App() {
  ...
  // Join one Socket.io room per workspace so the live-user badge is board-specific.
  useEffect(() => {
    if (!workspaceId) {
      setActiveUsers(0);
      return undefined;
    }
    const joinWorkspace = () => socket.emit('join_workspace', workspaceId);
    joinWorkspace();
  ...
  const handleUpvoteItem = async (id) => {
    try {
      const res = await fetch(`/api/retro-items/${id}/upvote`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ voterEmail: currentUser.email }),
      });
      const data = await res.json();
