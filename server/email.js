...
async function sendTeamInvite({ email, role, inviteUrl, invitedBy }) {
  ...
  try {
    ...
    if (!transporter) return { sent: false, inviteUrl, message: 'Email is not configured. Add RESEND_API_KEY and RESEND_FROM in Render to send invitations; the direct link is ready as a fallback.' };
    ...
    const renderFreeHint = !process.env.RESEND_API_KEY && smtpConfigured ? ' Render free services block SMTP ports, so configure Resend instead (RESEND_API_KEY and RESEND_FROM).' : '';
