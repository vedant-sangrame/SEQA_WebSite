...
router.post('/members/join', (req, res) => {
  ...
  if (existing) {
    ...
    const hasAdmin = db.data.members.some((member) => member.role === 'Admin');
    existing = {
...
router.post('/invite', async (req, res) => {
  ...
  const inviter = db.data.members.find((member) => member.email.toLowerCase() === normalizedInviterEmail);
  ...
  try {
    ...
    if (req.app.get('io')) {
      req.app.get('io').emit('member_joined', { email: recipientEmail, role: inviteRole });
    }
