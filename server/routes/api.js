...
router.post('/retro-items/:id/upvote', (req, res) => {
  ...
  if (!voterEmail) return res.status(400).json({ error: 'Voter email is required' });
  try {
    const item = db.data.retro_items.find((i) => i.id === id);
    if (!item) return res.status(404).json({ error: 'Item not found' });

    item.voted_by = Array.isArray(item.voted_by) ? item.voted_by : [];
    const existingVoteIndex = item.voted_by.indexOf(voterEmail);
    if (existingVoteIndex >= 0) {
      item.voted_by.splice(existingVoteIndex, 1);
    } else {
      item.voted_by.push(voterEmail);
    }
    item.upvotes = item.voted_by.length;
    db.save();
